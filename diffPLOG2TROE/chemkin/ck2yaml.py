# This file is modified from Cantera original ck2yaml.py file
# (see: https://github.com/Cantera/cantera/blob/main/interfaces/cython/cantera/ck2yaml.py) in order to generate a yaml
# file containing only the specification for the chemical reaction withouth the need to carry on the species
...
if yaml_version < yaml_min_version:
    raise RuntimeError(
        "The minimum supported version of ruamel.yaml is 0.17.16. If you installed ruamel.yaml from your operating"
        " system's package manager, please install an updated version using pip or conda."
    )
...
QUANTITY_UNITS = {"MOL": "mol", "MOLE": "mol", "MOLES": "mol", "MOLEC": "molec", "MOLECULES": "molec"}
...
class InputError(Exception):
    """
    An exception class for exceptional behavior involving Chemkin-format mechanism files. Pass a string describing the
...
class Reaction:
    ...
    def __str__(self):
        ...
        """
        return "{}{}{}".format(
            self._coeff_string(self.reactants),
    ...
    def to_yaml(cls, representer, node):
        ...
        if node.forward_orders:
            ...
            if any((float(x) < 0 for x in node.forward_orders.values())):
                ...
                logger.info("Negative reaction order for reaction " f"{node.index} ({node!s}).")
...
class KineticsModel:
    ...
    def reduce(self, output):
        ...
        """
        raise InputError("reduce is not implemented for objects of class {}", self.__class__.__name__)
...
class PDepArrhenius(KineticsModel):
    ...
    def reduce(self, output):
        ...
        for pressure, arrhenius in zip(self.pressures, self.arrhenius):
            rates.append(arrhenius.as_yaml([("P", "{0} {1}".format(pressure, self.pressure_units))]))
...
class Chebyshev(KineticsModel):
    ...
    def __init__(self, coeffs, *, Tmin, Tmax, Pmin, Pmax, quantity_units, **kwargs):
        KineticsModel.__init__(self, **kwargs)
        self.Tmin = Tmin
        self.Tmax = Tmax
        self.Pmin = Pmin
    ...
    def reduce(self, output):
        ...
        output["pressure-range"] = FlowList(["{0} {1}".format(*self.Pmin), "{0} {1}".format(*self.Pmax)])
...
def fortFloat(s):
    """
    Convert a string representation of a floating point value to a float, allowing for some of the peculiarities of
    allowable Fortran representations.
    """
...
class Parser:
    ...
    def entry(self, where="entry", kind="Error"):
        ...
        error_entry = "\n".join(lines).rstrip()
        return (
            f"{kind} while reading {where} in {self.files[-1]} starting on "
    ...
    def parse_composition(elements, nElements, width):
        ...
        for i in range(nElements):
            ...
            if not symbol:
                continue
            try:
                # Convert to float first for cases where ``count`` is a string like "2.00".
    ...
    @staticmethod
    def get_rate_constant_units(length_dims, length_units, quantity_dims, quantity_units, time_dims=1, time_units="s"):
        units = ""
        if length_dims:
            units += length_units
        if length_dims > 1:
    ...
    def read_NASA7_entry(self, lines, TintDefault, comments):
        ...
        if not composition:
            logger.error(
                self.entry("thermo entry") + "Error parsing elemental " f"composition for species '{species}'."
            )
            return species, None, {}

        for symbol in composition.keys():
            # Some CHEMKIN input files may have quantities of elements with more than 3 digits. This violates the
            # column-based input format standard, so the entry cannot be read and we need to raise a more useful error
            # message.
            if any(map(str.isdigit, symbol)) and symbol not in self.elements:
                logger.error(
                    self.entry("thermo entry") + "Error parsing elemental composition for species thermo "
        ...
        high_coeffs = [
            fortFloat(lines[i][j:k])
            for i, j, k in [(1, 0, 15), (1, 15, 30), (1, 30, 45), (1, 45, 60), (1, 60, 75), (2, 0, 15), (2, 15, 30)]
        ]
        low_coeffs = [
            fortFloat(lines[i][j:k])
            for i, j, k in [(2, 30, 45), (2, 45, 60), (2, 60, 75), (3, 0, 15), (3, 15, 30), (3, 30, 45), (3, 45, 60)]
        ]
        ...
        if Tint is None:
            if all(c == 0 for c in low_coeffs):
                ...
                logger.error(
                    self.entry("thermo entry") + "Only one temperature range defined but two distinct "
                    "sets of coefficients given\nin species thermo entry."
                )
                return species, None, {}

            thermo = Nasa7(Tmin=Tmin, Tmax=Tmax, Tmid=None, low_coeffs=coeffs, high_coeffs=None, note=note)
        else:
            thermo = Nasa7(Tmin=Tmin, Tmax=Tmax, Tmid=Tint, low_coeffs=low_coeffs, high_coeffs=high_coeffs, note=note)
    ...
    def parse_nasa7_section(self, lines):
            ...
            if len(line) >= 80 and line[79] == marker == "1":
                ...
                if name not in self.species_dict:
                    if self.skip_undeclared_species:
                        logger.debug(f"Skipping unexpected species '{name}' while " "reading thermodynamics entry.")
                        marker = "1"
                        comments = []
                        continue
                    else:
                ...
                if species.thermo is not None:
                    ...
                    if redundant_count <= 5 or self.verbose:
                        if self.permissive:
                            logger.warning(
                                "Ignoring redundant thermo data for species "
                                f"'{name}' starting on line {input_lineno+1} of "
                                f"{self.files[-1]}."
                            )
                        else:
                            logger.error(
                                self.entry("thermo entry") + f"Found additional thermo entry for species '{name}'. "
        ...
        if redundant_count > 5 and not self.verbose:
            ...
            logger.warning(
                f"Suppressed {redundant_count - 5} additional {kind} "
        ...
        for block in blocks:
            ...
            if any(entry[:, 2]):
                ...
                if self.permissive:
                    logger.warning(
                        self.entry("thermo data", "Unparsable lines") + "Lines could not be parsed as a NASA7 entry."
                    )
                else:
                    logger.error(
                        self.entry("thermo data", "Unparsable lines")
    ...
    def read_kinetics_entry(self, entry, surface):
        ...
        if "units" in entry.lower():
            for units in sorted(QUANTITY_UNITS, key=lambda k: -len(k)):
                pattern = re.compile(r"units *\/ *{} *\/".format(re.escape(units)), flags=re.IGNORECASE)
            ...
            for units in sorted(ENERGY_UNITS, key=lambda k: -len(k)):
                pattern = re.compile(r"units *\/ *{} *\/".format(re.escape(units)), re.IGNORECASE)
                m = pattern.search(entry)
                if m:
                    entry = pattern.sub("", entry)
        ...
        for token in reaction.split():
            ...
            try:
                ...
                try:
                    ...
                    logger.error(
                        self.entry("reaction") + f"Unexpected token '{token}' in reaction expression "
        ...
        if lhs:
            logger.error(
                self.entry("reaction") + "Failed to find reactant/product delimiter in reaction expression "
                f"'{original_reaction.strip()}'."
            )
        ...
        if third_body_name_r != third_body_name_p:
            logger.error(
                self.entry("reaction") + f"Third bodies do not match: '{third_body_name_r}' and "
                f"'{third_body_name_p}'."
            )
        ...
        if photon_p and reversible:
            if self.permissive:
                logger.warning(
                    self.entry("reaction", "Issue") + "Found a reversible reaction containing a product photon. "
                    "Converting to an\nirreversible reaction with the photon removed."
                )
            else:
                logger.error(
                    self.entry("reaction") + "Found a reversible reaction containing a product photon. "
        ...
        if rStoich < 1:
            logger.error(self.entry("reaction") + "No reactant species found in reaction equation.")

        length_dim = 3 * (rStoich - 1)
        quantity_dim = rStoich - 1
        kunits = self.get_rate_constant_units(length_dim, "cm", quantity_dim, quantity_units)
        klow_units = self.get_rate_constant_units(length_dim + 3, "cm", quantity_dim + 1, quantity_units)

        # The rest of the first line contains Arrhenius parameters
        arrhenius = Arrhenius(A=(A, kunits), b=b, Ea=(Ea, energy_units), parser=self)
        ...
        for line in lines[1:]:
            ...
            if "low" in line.lower():
                ...
                parsed = True
                C = tokens[1].split()
                coverages.append([C[0], fortFloat(C[1]), fortFloat(C[2]), fortFloat(C[3])])
        ...
        tests = [cheb_coeffs, pdep_arrhenius, low_rate, high_rate, third_body, surface]
        if sum(bool(t) for t in tests) > 1:
            logger.error(self.entry("reaction") + "Reaction contains parameters for more than one reaction type.")
            return reaction, revReaction

        if cheb_coeffs:
            if Tmin is None or Tmax is None:
                logger.error(self.entry("reaction") + "Missing TCHEB entry for Chebyshev reaction")
                return reaction, revReaction
            if Pmin is None or Pmax is None:
                logger.error(self.entry("reaction") + "Missing PCHEB entry for Chebyshev reaction")
                return reaction, revReaction
            if len(cheb_coeffs) != degreeT * degreeP:
                logger.error(
                    self.entry("reaction") + "Incorrect number of Chebyshev coefficients. Expected "
            ...
            reaction.kinetics = Falloff(high_rate=arrhenius, low_rate=low_rate, F=falloff, efficiencies=efficiencies)
        elif high_rate is not None:
            reaction.kinetics = ChemicallyActivated(
                high_rate=high_rate, low_rate=arrhenius, F=falloff, efficiencies=efficiencies
            )
        elif third_body:
            reaction.kinetics = ThreeBody(high_rate=arrhenius, efficiencies=efficiencies)
        elif reaction.third_body:
            logger.error(
                self.entry("reaction") + "Reaction equation implies pressure dependence but no alternate rate\n"
                "parameters (such as HIGH or LOW) were given."
            )
        elif surface:
            reaction.kinetics = SurfaceRate(
                rate=arrhenius, coverages=coverages, is_sticking=is_sticking, motz_wise=motz_wise
            )
        else:
    ...
    def load_extra_file(self, path):
        ...
        reserved = {"generator", "input-files", "cantera-version", "date", "units", "phases", "species", "reactions"}
        reserved &= set(yml.keys())
        if reserved:
            logger.error(
                f"The YAML file '{Path(path).name}' provided as '--extra' "
                "input\nmust not redefine reserved field name(s): " + ", ".join(f"{item!r}" for item in reserved)
            )
            return
        ...
        if "description" in yml:
            if isinstance(yml["description"], str):
                ...
                logger.error("The alternate description provided in " f"'{Path(path).name}'\nneeds to be a string.")
    ...
    def load_data_file(self, path: str, load_method: Callable, kind: str, **kwargs):
        ...
        try:
            ...
            logger.warning(f"\nERROR: Unable to parse '{path}' near line " f"{self.current_range[0]+1}:\n{err}\n")
            raise
    ...
    def load_chemkin_file(self, path, surface=False):
        ...
        for i, row in enumerate(lines):
            ...
            for section, pattern in sections.items():
                if m := pattern.match(line):
                    if current_section and not self.permissive:
                        logger.warning(
                            f"{current_section} section implicitly ended "
            ...
            if current_section in ("SPECIES", "ELEMENTS", "SITE"):
                ...
                logger.error(self.entry() + f"Section starts with unrecognized keyword '{line}'")
                break
        ...
        if "SITE" in found_sections:
            if not surface:
                logger.error(
                    self.entry("section") + "Surface phase input file must be specified using the "
    ...
    def parse_species_section(self, lines):
        ...
        for token in species:
            if token in self.species_dict:
                ...
                if redundant_count > 5:
                    continue
                if self.permissive:
                    logger.warning("Ignoring redundant declaration for " f"species '{species}'")
                else:
                    logger.error(
                        f"Found multiple declarations for species "
        ...
        if redundant_count > 5 and not self.verbose:
            kind = "warnings" if self.permissive else "errors"
            logger.warning(
                f"Suppressed {redundant_count - 5} additional {kind} about "
    ...
    def parse_site_section(self, lines):
        ...
        if site_density is None:
            logger.error(self.entry("SITE section") + "SITE section defined with no site density")
        ...
        for lineno, _, line, comment in lines:
            ...
            if len(entry) == entryLength:
                ...
                if label not in self.species_dict:
                    if self.skip_undeclared_species:
                        logger.debug(f'Skipping unexpected species "{label}" while ' "reading thermodynamics entry.")
                        continue
                    else:
                        # Add a new species entry
                ...
                if species.thermo is not None:
                    ...
                    if self.permissive:
                        ...
                        logger.error(
                            self.entry("thermo entry") + f"Found additional thermo entry for species '{label}'. Run "
    ...
    def parse_reactions_section(self, lines, surface):
        ...
        for token in lines[0, 2].upper().split():
            if token in ENERGY_UNITS:
                ...
                logger.error(self.entry("section") + f"Unrecognized token {token!r} on REACTIONS line")
        ...
        self.other_tokens.update(("(+{})".format(k), "falloff3b: {}".format(k)) for k in self.species_dict)
    ...
    def parse_transport_section(self, lines):
        ...
        for lineno, _, line, comment in lines:
            ...
            if speciesName in self.species_dict:
                if len(data) != 7:
                    logger.error(
                        self.entry("transport data") + "6 transport "
                ...
                if self.species_dict[speciesName].transport is None:
                    ...
                    if self.permissive:
                        logger.warning(
                            "Ignoring duplicate transport data for species "
                            f'"{speciesName}" on line {lineno+1} of "{self.files[-1]}".'
                        )
                    else:
                        logger.error(
                            self.entry("transport data") + f"Duplicate transport data for species '{speciesName}'. "
                            "Run ck2yaml again with the\n'--permissive' option to "
                            "ignore this redundant entry."
                        )
    ...
    def convert_mech(
        ...
        extra_file=None,
        out_name=None,
        single_intermediate_temperature=False,
        quiet=False,
        permissive=None,
        verbose=False,
        exit_on_error=False,
        ...
        parser.load_data_file(surface_file, parser.load_chemkin_file, "surface input", surface=True)
        ...
        if not quiet:
            ...
            logger.info(f"Mechanism contains {nSpecies} species and {nReactions} " "reactions.")
...
def convert(
    ...
    _, surface_names = Parser.convert_mech(
        ...
        extra_file,
        out_name,
        single_intermediate_temperature,
        quiet,
        permissive,
        verbose,
    )
...
def main(argv=None):
    ...
    if not input_file and not thermo_file:
        logger.error(
            'At least one of the arguments "--input" or "--thermo" '
    ...
    parser, surfaces = Parser.convert_mech(
        input_file,
        thermo_file,
        args.transport,
        args.surface,
        ...
        args.extra,
        out_name,
        args.single_intermediate_temperature,
        args.quiet,
        args.permissive,
        args.verbose,
        True,
    ...
    try:
        from cantera import Interface, Solution
    except ImportError:
        logger.warning("WARNING: Unable to import Cantera Python module. " "Output mechanism has not been validated")
        sys.exit(0)
...
def create_argparser():
    parser = argparse.ArgumentParser(
        description=("Convert Chemkin-format mechanisms to Cantera YAML input files"),
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "This option allows certain recoverable parsing errors (for example, "
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Chemkin-format chemistry input file, containing a list of all the "
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "If the INPUT file does not contain THERMO data, a separate file "
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "If the INPUT file does not contain TRANSPORT data, a separate file "
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "For surface mechanisms, the SURFACE file defines surface species "
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "This option specifies a YAML file which can be used to add to the "
    ...
    parser.add_argument(
        "--name",
        default="gas",
        help=("This specifies the name of the phase in the resulting YAML file. The " "default is **gas**."),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Disables the validation step, where the YAML mechanism is imported in "
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Specifies the OUTPUT file name. By default, the output file name is the "
