...
def plog_to_chemkin(plog: dict) -> str:
    line = "{}\t{} {} {}\n".format(plog["name"], plog["parameters"][-1][1], plog["parameters"][-1][2],
                                   plog["parameters"][-1][3])

    for i in range(0, len(plog["parameters"])):
        P, A, b, Ea = plog["parameters"][i][0], plog["parameters"][i][1], plog["parameters"][i][2], plog["parameters"][i][3]
...
def arrheniusbase_to_chemkin(arrhenius: dict) -> str:
    line = "{}\t{:.5E}  {:.5E}  {:.5E}\n".format(arrhenius["name"], arrhenius["parameters"][0],
                                                 arrhenius["parameters"][1], arrhenius["parameters"][2])
