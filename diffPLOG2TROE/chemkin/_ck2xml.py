...
class Species:
    ...
    def to_yaml(cls, representer, node):
        out = BlockMap([("name", node.label), ("composition", FlowMap(node.composition.items()))])
        if node.thermo:
    ...
    def __init__(self, *, parser, data, note=""):
        ...
        for i in range(1, len(data)):
            if abs(self.data[i - 1][0][1] - self.data[i][0][0]) > 0.01:
                logger.error(parser.entry("thermo entry") + "NASA9 polynomials contain non-adjacent temperature ranges")
    ...
    def to_yaml(cls, representer, node):
        ...
        if node.forward_orders:
            ...
            if any((float(x) < 0 for x in node.forward_orders.values())):
                ...
                logger.info(f"Negative reaction order for reaction {node.index} ({node!s}).")
...
class TransportData:
    ...
    def __init__(
        self, parser, label, geometry, well_depth, collision_diameter, dipole_moment, polarizability, z_rot, note=""
    ):
        try:
            ...
            try:
                ...
                logger.error(
                    parser.entry("transport data") + f"Invalid geometry flag '{geometry}' for species '{label}'. "
            ...
            if geometry == int(geometry):
                ...
                if not parser.permissive:
                    logger.info(
                        f"Incorrect geometry flag syntax '{geometry}' for "
                ...
                logger.error(
                    parser.entry("transport data") + f"Invalid float geometry flag '{geometry}' for species '{label}'. "
                    "Flag must be an integer."
                )
                return
        if geometry not in (0, 1, 2):
            logger.error(
                parser.entry("transport data") + f"Invalid geometry flag value '{geometry}' for species '{label}'. "
...
class Parser:
    ...
    def read_NASA7_entry(self, lines, TintDefault, comments):
        ...
        if not composition:
            logger.error(self.entry("thermo entry") + f"Error parsing elemental composition for species '{species}'.")
            return species, None, {}
    ...
    def read_kinetics_entry(self, entry, surface):
        ...
        if third_body_name_r != third_body_name_p:
            logger.error(
                self.entry("reaction") + f"Third bodies do not match: '{third_body_name_r}' and '{third_body_name_p}'."
            )
    ...
    def load_extra_file(self, path):
        ...
        if "description" in yml:
            if isinstance(yml["description"], str):
                ...
                logger.error(f"The alternate description provided in '{Path(path).name}'\nneeds to be a string.")
    ...
    def load_data_file(self, path: str, load_method: Callable, kind: str, **kwargs):
        ...
        try:
            ...
            logger.warning(f"\nERROR: Unable to parse '{path}' near line {self.current_range[0] + 1}:\n{err}\n")
            raise
        ...
        for token in species:
            if token in self.species_dict:
                ...
                if redundant_count > 5:
                    continue
                if self.permissive:
                    logger.warning(f"Ignoring redundant declaration for species '{species}'")
                else:
                    logger.error(
