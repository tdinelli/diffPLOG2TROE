...
def fit_smooth_nasa7_coefficients(species: Species, T_mid: float | None = None) -> Species:
    ...
    if T_mid is not None:
        ...
        fitted_coeffs, _, _ = _fit_spline_to_heat_capacity(species, temperatures, knot_temperature)
        ...
        # Pre-compute temperature powers and Cp/R once - these are constant across all candidate knots
        ...
        while knot_candidate <= T_max_search:
            # Reuse pre-computed T_powers and y to avoid redundant computation
            fitted_coeffs, X, _ = _fit_spline_to_heat_capacity(
                species,
                temperatures,
                knot_candidate,
                T_powers=T_powers,
            ...
            error = _compute_relative_fit_error(fitted_coeffs, X, y)
        ...
        # Final fit with optimal T_mid (reuse pre-computed values)
        fitted_coeffs, _, _ = _fit_spline_to_heat_capacity(species, temperatures, best_T_mid, T_powers=T_powers, y=y)
...
def _fit_spline_to_heat_capacity(
    ...
    T_powers: Float64[Array, "n 6"] | None = None,
    y: Float64[Array, "n"] | None = None,
) -> tuple[Float64[Array, "6"], Float64[Array, "n 6"], Float64[Array, "n"]]:
    """
    ...
    T_powers : Float64[Array, "n 6"], optional
        Pre-computed temperature powers. If None, computed from temperatures.
    y : Float64[Array, "n"], optional
        Pre-computed Cp/R values. If None, computed from species.

    Returns
    -------
    tuple[Float64[Array, "6"], Float64[Array, "n 6"], Float64[Array, "n"]]
        (fitted_coeffs, X, y) where:
        - fitted_coeffs: [a0, a1, a2, a3, a4, alpha]
        - X: design matrix for reuse
        - y: Cp/R values for reuse
    """
    if y is None:
        y = species.cp_over_r(temperatures)

    X = _build_spline_basis_matrix(temperatures, knot_temperature, T_powers)

    # Assemble and solve the normal equations like OpenSMOKE: solve (X^T X) * params = X^T * y
    XTX = X.T @ X
    XTy = X.T @ y
    fitted_coeffs = jnp.linalg.solve(XTX, XTy)

    return fitted_coeffs, X, y
...
def _compute_relative_fit_error(
    ...
    return jnp.sum(jnp.abs(cp_fitted - y) / jnp.abs(y))
...
def _build_spline_basis_matrix(
    ...
    knot_temperature : Float64[Array, ""]
        Intermediate temperature for spline correction [K]
    T_powers : Float64[Array, "n 6"], optional
        Pre-computed temperature powers [1, T, T^2, T^3, T^4, LOG(T)].
        If None, will be computed from temperatures. Providing this avoids
        redundant computation when building multiple basis matrices for the
    ...
    X = jnp.concatenate([T_powers[:, :5], spline_correction[:, None]], axis=1)
