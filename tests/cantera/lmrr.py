...
P_range = np.array([0.1, 1, 10, 100])
k_matrix = np.zeros((len(P_range), len(T_range)))
k_troe = np.zeros((len(P_range), len(T_range)))
k_chemkin = np.zeros((len(P_range), len(T_range)))
...
for t_idx, t in enumerate(T_range):
    for p_idx, p in enumerate(P_range):
        gas.TPX = t, p * ct.one_atm, "H2:0.1, HE:0.5, AR:0.4"
        k_matrix[p_idx, t_idx] = gas.forward_rate_constants[7]
        k_troe[p_idx, t_idx] = gas.forward_rate_constants[4]
        k_chemkin[p_idx, t_idx] = kf_chemkin(gas)
...
for i, p in enumerate(P_range):
    col = colors[i]
    plt.semilogy(T_range, k_matrix[i, :], color=col, alpha=0.5)
    # plt.semilogy(T_range, k_troe[i, :], "b--")
    plt.semilogy(T_range, k_chemkin[i, :], "-.", color=col)
