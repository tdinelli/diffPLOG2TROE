...
class TestMixtureRules(unittest.TestCase):
    def setUp(self):
        self.T_range = jnp.linspace(300, 3000, 10)
        # self.P_range = jnp.logspace(jnp.log10(0.01), jnp.log10(100), 300)
        self.P_range = jnp.array([1])
        ...
        ar_specific_rate = FallOff(
            name="H+O2(+AR)=HO2(+AR)",
            hpl_parameters={"A": 4.650e12, "n": 0.44, "Ea": 0.0},
            lpl_parameters={"A": 6.81e18, "n": -1.2, "Ea": 0.0},
            falloff_parameters={"A": 0.7, "T3": 1e-30, "T2": 1e30, "T1": 1e30},
            falloff_type="troe",
        )
        he_specific_rate = FallOff(
            name="H+O2(+HE)=HO2(+HE)",
            hpl_parameters={"A": 4.650e12, "n": 0.44, "Ea": 0.0},
            lpl_parameters={"A": 9.192, "n": -1.2, "Ea": 0.0},
        ...
        self.extended_falloff_reaction = MixtureRule(
            ...
            explicit_rate_constants={"Ar": ar_specific_rate, "He": he_specific_rate},
            efficiencies=[CollisionEfficiency(name="H2", value=1.30), CollisionEfficiency(name="H2O", value=10.00)],
            linear=True,
